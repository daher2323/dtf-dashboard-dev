...
| Logistics (throughput)   | `renderLogistics` → `_logiTilesHTML` + `_logiTrendHTML` + `_logiLogHTML` (`logiPeriodSel`, `logiTrendGran`, `logiTrendMetric`) |
...
- **Logistics sheet** (`CSV_LOGISTICS`, `parseLogistics`): a per-line-item ledger (one row per lot shipped). Parsed **positionally** (`LOGI_COL` index map via `parseCSVGrid`) because row 0 is stray junk and the real header is row 1. Two quirks the parser handles and must keep handling: (1) the `Date` column is sparse and **forward-fills** — a blank date inherits the most recent date above it; (2) the sheet trails ~800 empty rows that still compute `Total Cost = $0.00`, so a row counts only if it has a `Product Name` or `Lot #`. Out-of-range years (2020–2035 guard) are treated as blank so a typo'd date forward-fills instead of creating a bogus bucket. `Product Name` follows the same `"Customer - Product"` convention.
- **Logistics tab is a throughput view** (rebuilt from the earlier cost ledger; cost fields are still parsed but deliberately unreferenced by the view). Config lives next to `LOGI_COL`: `DTF_HOLIDAYS` (empty → holidays count as business days), `PARCEL_CARRIERS`, `PICKUP_MARKERS`. Business-day rates use `isBusinessDay`/`countBusinessDays`; periods (`Today`/`This week`/`This month`) via `periodBounds`, with like-for-like prior comparison on **business days elapsed** (`priorPeriodBounds`). **Shipment grouping** (`groupLotsToShipments`): group lot rows by ship date + customer, split only when a group has ≥2 distinct non-blank BOLs — there is no shipment ID in the source, so shipment counts are **approximate** (2026 ≈ 1,716 lots → ~528 shipments); lots/pallets/boxes are exact. **Pallets:** a blank pallet cell is treated as a genuine zero for every carrier (`palletsForRow` always `recorded:true`) — decided because the Carrier column is blank on ~66% of rows, so a per-row "unrecorded" flag would be misleading. `carrierClass` reads **both** Carrier and Broker (pickup lives in Broker). **Lot numbers stay strings** (`lotKey` — never `Number()`). The trend chart (`charts.logiTrend`) is mutated in place on toggle (`_updateLogiTrend`), only destroyed on full render.
