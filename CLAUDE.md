...
**The Customer Breakdown renders every customer, always.** Top 10/25/All and the name search box (`ooCustSearch`) decide row visibility in the DOM via `filterOOCustomers` — each customer's summary row + drilldown is wrapped in one `.oo-cust-row` carrying `data-oo-cust` (lowercased name) and `data-oo-rank`. Don't "optimize" it back to slicing the list before render: typing would then rebuild `#content` (and the Chart.js charts) on every keystroke and fight the caret. A search query **overrides** the limit rather than intersecting with it — reaching a customer without switching to All is the whole point — and initial visibility is computed at render time so a refresh repaints already filtered.

**Per-customer Open Orders export** (`exportOpenOrdersForCustomer`, reached from the download icon on each Customer Breakdown row, the labelled button inside that row's drilldown, and each Labels-tab brand header via `_ooExportBtn`). Unlike the other exports, this one deliberately does **not** mirror the screen: it rebuilds the brand's full open book from `openOrdersPowder`/`openOrdersCapsule` (`_ooBrandRows`) so an All/Powder/Capsule or Top 10/25 toggle can't silently drop half a customer's orders from a report that goes out to that customer. Consequence to keep in mind: its unit total **exceeds** the Customer Breakdown's figure, because the breakdown counts production units only (`isSamplePackOrBulk` rows excluded — that classifier needs `Order Amount` of exactly 1, or `Bottles/Jars` = `N/A` **plus** a stick/sample/bulk signal in the product name, since `N/A` alone only means "not a bottle or jar" and was demoting gusset-bag production orders to bulk) — the sheet's header line states the basis so the gap doesn't read as a bug. `Labels on Hand` / `Labels Short` come from the sheet's `Labels` column (a count in house), which is populated on **every** open row — so both columns are filled on every row, `0 short` included. They were briefly restricted to `Not Enough Labels` rows and the resulting sea of blanks read as broken data. `_ooLabelsOnHand` only does arithmetic when the cell is numeric, so a text note (`"Unlabeled"`) never becomes a fake shortfall. That column is also the least reliable one on the sheet — labels routinely sit in receiving before anyone updates it — so both sheets carry `LABEL_PROVENANCE`, which says the counts are as-of-export and that a `0` may mean "not logged yet". Don't dress those numbers up as authoritative. The second sheet (`Labels Needed`) is omitted entirely when nothing is blocked.

**Optional columns disappear rather than ship empty** (`_ooBuildSheet` + the `optional:true` flag on a column model). Every cell is materialized first, then any optional column with no value on any row of *that customer's* book is dropped from the workbook: roughly half our orders carry no customer PO #, and a column of blanks reads as missing data rather than "doesn't apply here". Same mechanism retires `Labels Short` for a customer whose label cells are all text (Twisted Dough's `"Unlabeled"`). `total` on a column definition puts it in the closing TOTAL row; the `TOTAL` label lands in the first surviving column, which is always text. Columns are customer-facing and ordered `Invoice / Estimate # · PO # · Product · Order Amount · Weeks on Books · Planning Status · Labels on Hand · Labels Short`; **`Promise Date / Notes` and `Label Status Update` are deliberately excluded** — they're internal hand-written scratch notes and must not go out to a customer. Don't add them back.
...
- **Material expiry** (`parseInvDate`, `expiryPosition`, `renderExpiryCell`, `matLotTableHTML`). The inventory feed is one row per received lot and now carries `EXP. Date`, `Receiving Date`, `Status`, `Locations`, `Supplier/MFG`, `Lot / Batch`. `parseInventory` keeps those as `lotRows` for lots **holding stock only** (a spent lot's date is not a risk). `expiryPosition(parts, ratePerMonth)` walks a material's lots first-expiry-first-out and returns `expiredKg` / `kg30` / `kg60` / `kg90` / `riskKg` / `noExpiryKg` / `badDateLots`, where `riskKg` is the quantity the run rate cannot reach before its lot's date. Four invariants, all load-bearing — don't "simplify" them away:
  - **Never infer a date.** `parseInvDate` rejects blanks, malformed values, out-of-range years and impossible dates (`2/31`, `02/29/2026`). The sheet contains ~24 typos (`27/03/2029`, `1001/2028`, `Not Found`); they surface as `bad date`, visually distinct from an empty cell, because one needs a typo fixed and the other needs a date entered.
  - **Undated stock is never "safe" stock.** It goes to `noExpiryKg` and is excluded from `riskKg`, and the page states coverage explicitly (`Expiry coverage` line). This mattered enormously when the feed only carried 44% of the kg.
  - **Already-expired stock is reported apart from at-risk stock** (`expiredKg` vs `riskKg`) and excluded from the FEFO walk — it is a disposition decision that already landed, not a forecast.
  - **Zero-usage rows get the walk too**, at rate 0, so every dated kg counts as at risk. 87% of the at-risk tonnage is material nothing has drawn in six months; clicking the `Expiring` segment therefore ticks `matShowZeroUsage` (visibly, not by bypassing it) or the filter would under-report by design.
  Also: FEFO is an assumption about picking, stated in the lot table's footnote; `EXPIRY_RISK_MIN_KG` (1 kg) keeps remnant dust out of `riskKg`; the on-screen lot table caps at `MAT_LOT_CAP` and says so, while the export's `Lots By Expiry` sheet is uncapped.
- **FEFO review** (`computeFefoMisses`, memoized on pick-row + lot-index counts). The pick sheet records the T# pulled; a lot's balance at any past date reconstructs as *current stock + every pick after that date*, so a pick is a FEFO miss when a sibling lot of the same part had an earlier expiry, was already received, was still in date, and still held ≥ `FEFO_MIN_KG`. Two exclusions are load-bearing: a sibling **already expired** at pick time is not a miss (you can't use expired material), and sub-kilo remnants don't count. `inventoryLotIndex` exists for this — it indexes **every** lot including depleted ones, because the lot that was *picked* is usually spent and absent from `lotRows`. The reviewable unit is the **lot, not the pick**: ~6,300 pick-level misses collapse to ~757 stranded lots, and reporting those is still useless (dominated by high-turnover aminos — one L-Citrulline lot is skipped 250× and will be consumed 400 days early). The review list therefore intersects "gets skipped" with "actually at risk": expired or within `FEFO_REVIEW_HORIZON_DAYS` (90) **and** ≥ `FEFO_REVIEW_MIN_KG` (25 kg) → ~39 lots. Don't loosen that intersection without re-checking the count; the value is that it fits on one screen. It under-reports by design (only ~52% of picked T#s remain in the feed) and knows nothing of write-offs or holds, so every surface says *review*, never *violation*.
